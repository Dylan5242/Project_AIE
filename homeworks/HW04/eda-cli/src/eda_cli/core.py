...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    - has_constant_columns: есть константные признаки;
    - has_high_cardinality_categoricals: категориальные признаки с высокой кардинальностью;
    - has_suspicious_id_duplicates: дубликаты в идентификаторах;
    - has_many_zero_values: числовые признаки с большим числом нулей;
    - quality_score: агрегированный скор качества.
    """
    flags: Dict[str, Any] = {}

    # --- Базовые эвристики размера датасета ---
    ...
    # --- Константные колонки ---
    # Если в колонке только одно уникальное значение, она не несёт информации
    flags["has_constant_columns"] = any(
        col.unique == 1 for col in summary.columns
    )

    # --- Высокая кардинальность категориальных признаков ---
    # Категориальный признак считается проблемным, если доля уникальных значений
    # слишком велика по отношению к числу строк
    high_cardinality_threshold = 0.3
    flags["has_high_cardinality_categoricals"] = any(
        (not col.is_numeric) and summary.n_rows > 0 and (col.unique / summary.n_rows) > high_cardinality_threshold
        for col in summary.columns
    )

    # --- Дубликаты идентификаторов ---
    # Идентификаторы ожидаются уникальными; ищем колонки с именем *_id
    id_like_columns = [
        col for col in summary.columns if col.name.lower().endswith("_id")
    ]
    flags["has_suspicious_id_duplicates"] = any(
        col.unique < summary.n_rows for col in id_like_columns
    )

    # --- Большая доля нулей в числовых колонках ---
    # Если нули доминируют, признак может быть деградировавшим или бинарным
    zero_share_threshold = 0.5
    flags["has_many_zero_values"] = any(
        col.is_numeric
        and summary.n_rows > 0
        and col.non_null > 0
        and (col.non_null - col.unique + 1) / col.non_null > zero_share_threshold
        for col in summary.columns
    )

    # --- Агрегированный скор качества ---
    score = 1.0
