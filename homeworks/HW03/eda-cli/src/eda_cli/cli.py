...
def report(
...
)  # NEW: колонки с высокой долей пропусков
...
)  # NEW: используем значение из CLI
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        report_title = title or "EDA-отчёт"  # NEW: если заголовок не задан, используем дефолт
...
#доработать попозже
        ...
        if missing_df.empty:
            ...
            if not problematic_missing_cols.empty:
                f.write("Колонки с проблемной долей пропусков:\n\n")
                for col in problematic_missing_cols.index:
                    share = problematic_missing_cols.loc[col, "missing_share"]
                    f.write(f"- `{col}`: {share:.1%}\n")
                f.write("\n")
            else:
                f.write("Колонок с проблемной долей пропусков не найдено.\n\n")
#конец блока доработки
        ...
        if not top_cats:
            ...
            f.write(
                f"Для каждой колонки сохранены top-{top_k_categories} значений. "
        ...
        f.write(
            f"Построены гистограммы для не более чем {max_hist_columns} числовых колонок.\n"
        )  # NEW
